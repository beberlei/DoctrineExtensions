...
class TrigTest extends \DoctrineExtensions\Tests\Query\MysqlTestCase
{
    public function setUp()
    {
        ...
        $this->sclrAlias = $this->getColumnAlias('sclr');
    }
    ...
    public function testSin()
    {
        ...
        $sql = "SELECT SIN(RADIANS(b0_.latitude)) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());

        $dql = "SELECT SIN(p.latitude * p.longitude) FROM {$this->entity} p";
        $q = $this->entityManager->createQuery($dql);

        $sql = "SELECT SIN(b0_.latitude * b0_.longitude) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());

        $dql = "SELECT SIN(RADIANS(p.latitude) * RADIANS(p.longitude)) FROM {$this->entity} p";
        $q = $this->entityManager->createQuery($dql);

        $sql = "SELECT SIN(RADIANS(b0_.latitude) * RADIANS(b0_.longitude)) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
        ...
        $longAlias = $this->getColumnAlias('longitude', 2);
        $latAlias = $this->getColumnAlias('latitude', 3);

        $sql = "SELECT b0_.id AS $idAlias, b0_.created AS $createdAlias, b0_.longitude AS $longAlias, b0_.latitude AS $latAlias FROM BlogPost b0_ WHERE b0_.longitude = SIN(RADIANS(b0_.latitude)) * RADIANS(b0_.longitude)";
        $this->assertEquals($sql, $q->getSql());

        $dql = "SELECT p FROM {$this->entity} p WHERE SIN(RADIANS(p.latitude)) * SIN(RADIANS(p.longitude)) = 1";
        $q = $this->entityManager->createQuery($dql);

        $sql = "SELECT b0_.id AS $idAlias, b0_.created AS $createdAlias, b0_.longitude AS $longAlias, b0_.latitude AS $latAlias FROM BlogPost b0_ WHERE SIN(RADIANS(b0_.latitude)) * SIN(RADIANS(b0_.longitude)) = 1";
        $this->assertEquals($sql, $q->getSql());
        ...
        $sql = "SELECT SIN(RADIANS(b0_.latitude)) * SIN(RADIANS(b0_.longitude)) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
    }
    ...
    public function testAcos()
    {
        ...
        $sql = "SELECT ACOS(SIN(RADIANS(b0_.latitude)) + SIN(RADIANS(b0_.longitude))) * 1 AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
    }
    ...
    public function testAtan()
    {
        ...
        $sql = "SELECT ATAN(b0_.latitude, b0_.longitude) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
    }
    ...
    public function testAtan2()
    {
        ...
        $sql = "SELECT ATAN2(b0_.latitude, b0_.longitude) AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
    }
    ...
    public function testCosineLaw()
    {
        ...
        $sql = "SELECT ACOS(SIN(0) * SIN(RADIANS(b0_.latitude)) + COS(0) * COS(RADIANS(b0_.latitude)) * COS(RADIANS(b0_.longitude) - 0)) * 6371 AS {$this->sclrAlias} FROM BlogPost b0_";
        $this->assertEquals($sql, $q->getSql());
    }
    ...
    protected function _getFirstSqlQuery($func)
    {
        ...
        return "SELECT b0_.id AS $idAlias, b0_.created AS $createdAlias, "
        . "b0_.longitude AS $longAlias, b0_.latitude AS $latAlias "
    ...
    {
        return "SELECT " . $func . "(b0_.latitude) AS {$this->sclrAlias} FROM BlogPost b0_";
    }
}
